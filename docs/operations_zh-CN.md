...
用 Python 自带的 `compression.zstd`（3.14+），否则调本地 `zstd` 二进制。能解出来的部分照常
保留，并报告有多少份日志没有解到结尾。保留半份是安全的：frame 只追加不重写，
...
两种情况贡献的可解前缀完全相同，区分只会改变一条提示的措辞。真正的信号是：没有 dsh 会话在跑，
这个计数却一直不归零，那就是真的损坏了。两种解码器都没有的机器会报告有多少份日志完全读不了，
