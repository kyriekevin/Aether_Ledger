...
- 签名推送器最多等待 30 秒，并在 pull **和**读取数据文件的整个过程中持锁：写入脚本是
  逐个替换各 agent 的 JSON 文件的，不持锁读取可能读到新旧混合的一份。等不到锁就整轮
  跳过，而不是绕开写入方去读；下一次 `WatchPaths` 触发或 15 分钟定时会重新推送。
...
东西"而删除。`git branch -D` 会连同分支的 reflog 一起删掉，那些提交会变成游离对象，
只在 Git 清理（gc/prune）之前还能捞回来。
